...
fn reset_game(l_paddle:&mut paddle::Paddle, r_paddle:&mut paddle::Paddle, screen_width:i32, screen_height:i32) {
...
fn handle_win_condition(l_paddle:&mut paddle::Paddle, r_paddle:&mut paddle::Paddle, screen_width:i32, screen_height:i32) {
    reset_game(l_paddle, r_paddle, screen_width, screen_height);
}

fn handle_l_win_condition(l_paddle:&mut paddle::Paddle, r_paddle:&mut paddle::Paddle, gamestate:&mut gamestate::Gamestate, screen_width:i32, screen_height:i32) {
    *gamestate = gamestate::Gamestate::LWON;
    handle_win_condition(l_paddle, r_paddle, screen_width, screen_height);
}

fn handle_r_win_condition(l_paddle:&mut paddle::Paddle, r_paddle:&mut paddle::Paddle, gamestate:&mut gamestate::Gamestate, screen_width:i32, screen_height:i32) {
...
fn check_and_handle_goal(ball:&mut ball::Ball, l_paddle:&mut paddle::Paddle, r_paddle:&mut paddle::Paddle, gamestate:&mut gamestate::Gamestate, screen_width:i32, screen_height:i32) {
    if (ball.pos.x + ball.radius) < 0.0 {
        ...
        if r_paddle.points >= 9 {
            handle_r_win_condition(l_paddle, r_paddle, gamestate, screen_width, screen_height)
        }
    }
    if (ball.pos.x - ball.radius) > screen_width as f32 {
        ...
        if l_paddle.points >= 9 {
            handle_l_win_condition(l_paddle, r_paddle, gamestate, screen_width, screen_height)
        }
    }
}
    ...
    while rl.window_should_close()==false {
        match gamestate {
            ...
            gamestate::Gamestate::GAME => {
                ...
                l_paddle.update(&mut rl, &ball, against_ai, true, SCREEN_HEIGHT);
                r_paddle.update(&mut rl, &ball, against_ai, false, SCREEN_HEIGHT);
